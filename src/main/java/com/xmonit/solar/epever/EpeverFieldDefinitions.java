...
public enum EpeverFieldDefinitions {
    ...
    MANAGEMENT_MODES_OF_BATTERY_CHARGING_AND_DISCHARGING(0x9070, (cc,addr) -> new CodesField(addr, 1, new HexCodes("Battery Mode").add(0x00,"Voltage Compensation").add(0x01,"SOC"))),
    ...
    public boolean isStatistic() {
        return isDiscreteInputBacked() || isInputRegisterBacked();
    }

    public boolean isRegisterBacked() {
        return isInputRegisterBacked() || isHoldingRegisterBacked();
    }

    public boolean isInputRegisterBacked() {
        return registerAddress >= 0x3000 && registerAddress < 0x9000;
    }

    public boolean isHoldingRegisterBacked() {
        return registerAddress >= 0x9000;
    }

    public boolean isBooleanBacked() {
        return isCoilBacked() || isDiscreteInputBacked();
    }

    public boolean isCoilBacked() {
        return registerAddress < 0x2000;
    }

    public boolean isDiscreteInputBacked() {
        return registerAddress >= 0x2000 && registerAddress < 0x3000;
    }
